<template>
  <div id="app">
      ...
      <div class="header-section">
        ...
        <div class="header-controls">
          ...
          <div class="button-group">
            ...
            <Button @click="showSupplierManagement = true" variant="primary" icon="fas fa-truck">
              <span>Suppliers</span>
            </Button>
            <Button @click="exportInventory" variant="info" icon="fas fa-download">
              <span>Export</span>
            </Button>
            <Button @click="handleLogout" variant="danger" icon="fas fa-sign-out-alt">
              <span>Logout</span>
            </Button>
          </div>
        </div>
    ...
    <SupplierManagementModal
      :is-open="showSupplierManagement"
      :suppliers="suppliers"
      :products="inventory"
      @close="showSupplierManagement = false"
...
import type { Product, Supplier, SupplierOrder, POSTransaction } from '@/types'
...
import SupplierManagementModal from '@/components/modals/SupplierManagementModal.vue'
...
// Use composables
const { inventory, categories, suppliers, branches, loadInitialData, addProduct, updateProduct, deleteProduct: deleteProductFromAPI, addInventoryHistory } = useInventoryData()
...
const showSupplierManagement = ref(false)
...
const lowStockCount = computed(() => {
  return inventory.value.filter((item: Product) => item.stock_quantity <= (item.min_stock_level ?? 10)).length
})

const outOfStockCount = computed(() => {
  return inventory.value.filter((item: Product) => item.stock_quantity <= 0).length
})

const totalInventoryValue = computed(() => {
  return inventory.value.reduce((sum: number, item: Product) => sum + item.price * (item.stock_quantity ?? 0), 0)
})

const averageStockDays = computed(() => {
  return inventory.value.length > 0 ? (inventory.value.reduce((sum: number, item: Product) => sum + (item.dailySales || 2), 0) / inventory.value.length).toFixed(1) : 0
})

const inventoryTurnoverRate = computed(() => {
  const totalCost = inventory.value.reduce((sum: number, item: Product) => sum + (item.cost || 0), 0)
...
const saveProduct = async (product: Partial<Product>) => {
  try {
    console.log('📝 Saving product with data:', JSON.stringify(product, null, 2));
    
    if (editingProduct.value) {
      console.log('Updating existing product ID:', editingProduct.value.id);
      await updateProduct(editingProduct.value.id, product)
      showNotification('success', 'Product updated successfully')
    } else {
      console.log('Creating new product');
      const newProductData = product as Omit<Product, 'id'>
...
const addSupplier = async (supplier: Omit<Supplier, 'id'>) => {
  try {
    // TODO: Implement supplier API endpoints if needed
    showNotification('info', `Supplier ${supplier.name} functionality will be integrated with backend API`)
  } catch (error) {
    showNotification('error', 'Failed to add supplier')
  }
}

const removeSupplier = async (_supplierId: number) => {
  try {
    // TODO: Implement supplier API endpoints if needed
    showNotification('info', 'Supplier removal functionality will be integrated with backend API')
  } catch (error) {
    showNotification('error', 'Failed to remove supplier')
  }
}
...
const addToCart = (product: Product) => {
  if (!replenishmentCart.value.find((p: Product) => p.id === product.id)) {
...
const exportInventory = () => {
  try {
    ...
    showNotification('success', 'Export functionality will be implemented soon')
  } catch (error) {
...
const handleAddProduct = (newProduct: Product) => {
  // The addProduct method will handle API call, no need to manually push
...
onMounted(async () => {
  try {
    ...
    showNotification('info', `Inventory loaded: ${inventory.value.length} products`)
  } catch (error) {
